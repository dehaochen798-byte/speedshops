	...
	<view class="container">
		...
		<view class="vcartlist" v-for="(item,index) in cartlist">
			...
			<checkbox-group @change="ckchild($event,item)">
				<view class="vcartlist-goods" v-for="(gitem,gindex) in item.goodsDetail">
					...
					<view class="goods-text">
						...
						<view class="goods-text-price">
							...
							<view style="width: 160rpx;color:#333">
								<ControllerBuyNumBtn :myid="gitem.gdid" :max="item.restBuyNum" :num="gitem.buyNums"
									@toBuyNum="setBuyNumMethod" @toBuyId="getBuyIdMethod"></ControllerBuyNumBtn>
							</view>
						</view>
					</view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			mydel() {
				this.cartlist.forEach(mycartitem => {
					mycartitem.goodsDetail = mycartitem.goodsDetail.filter(mycartDetailitem => mycartDetailitem
						.checked == false)
				})
			...
			updateCartList(){
				...
				if(this.cartlist==[] || this.cartlist==null || this.cartlist==undefined || this.cartlist.length==0){
					uni.request({
						url:app.globalData.baseUrl + 'cart/emptycartlist/' + app.globalData.cid,
						method:"GET",
						success: (res) => {
			...
			resetGoodsSelected(){
				...
				for (var i = 0; i < this.oldCartList.length; i++) {
					...
					for (var j = 0; j < this.oldCartList[i].goodsDetail.length; j++) {
						...
						this.cartlist[i].goodsDetail[j].checked=this.oldCartList[i].goodsDetail[j].checked
					}
				}
			},
				...
				this.cartlist.forEach(mycartitem => {
					...
					mycartitem.goodsDetail.forEach(mycartDetailitem => {
						...
						if (mycartDetailitem.gdid == myid) {
							...
							if(mycartDetailitem.checked){// 如果是选中状态，重新计算
								// 计算合计价钱
								this.compterAllPrice()
							} 
						}
