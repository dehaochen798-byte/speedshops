  ...
  <view>
    <view class="goods-page">
      ...
      <swiper class="goodsswiper" autoplay="true">
        <swiper-item v-for="(item, index) in curgoods.icons" :key="index">
          <image :src="item" mode="widthFix" class="swiper-img"></image>
        </swiper-item>
      </swiper>
    </view>
    ...
    <view class="container">
      ...
      <view class="goods-select container-moudle">
        <view class="goods-select-p" @click="openSafe(curgoods.cgid)">
          <view class="goods-select-type"> 保障 </view>
        ...
        <view class="goods-select-p" @click="openBuyGoods">
          <view class="goods-select-type"> 选择 </view>
          <view class="goods-select-title">
            <view class="goods-select-left">
              <!-- 商品缩略图 -->
              <image
                v-if="curgoods.selectType == 2"
                :src="item.gdpic"
                v-for="(item, index) in curgoods.goodsDetailList"
                class="goods-select-left-pics-item"
              ></image>
              <!-- 商品缩略图 -->
              <!-- 商品标题 -->
              <view
                v-if="curgoods.selectType == 1"
                class="goods-select-left-gname"
                v-for="(item, index) in curgoods.goodsDetailList"
              >
        ...
        <view class="goods-select-p">
          <view class="goods-select-type"> 活动 </view>
          <view class="goods-select-title">
        ...
        <view class="goods-select-p">
          <view class="goods-select-type"> 物流 </view>
          <view class="goods-select-title">
            现货 1小时内送到货，
            <text v-if="curgoods.postprice <= 0"> 包邮 </text>
      ...
      <view class="container-moudle">
        <Evaluate
          :evaluateCount="curgoods.evaluateCount"
          :evaluateList="curgoods.evaluateList"
        ></Evaluate>
      </view>
      <!-- 商品评价 -->
    </view>
    ...
    <view class="footer">
      ...
      <view class="nav-item" @click="openCart">
        ...
        <text>购物车</text>
        <view class="cart-num" v-if="cartAllTotalNums > 0">{{
          cartAllTotalNums
        }}</view>
      </view>
      <view class="nav-item">
    ...
    <uni-popup ref="safePopup" type="bottom">
      <Safeguard
        :safeList="curgoods.safeguardList"
        @colseSafePopup="colseSafePopupMethod"
      ></Safeguard>
    </uni-popup>
    <!-- 底部弹出的保障信息窗口 -->
    ...
    <uni-popup ref="buyGoodsPopup" type="bottom">
      <BuyGoodsInfo
        :curgoods="curgoods"
        :curcartlist="cartlist"
        @colsebuyGoodsPopup="colsebuyGoodsPopupMethod"
      ></BuyGoodsInfo>
    </uni-popup>
    <!-- 底部弹出选择购买商品的详细信息 -->
  </view>
...
export default {
  ...
  onLoad(options) {
    ...
    // 获得当前商品信息
    if (
      app.globalData.curGoods.gid != undefined &&
      app.globalData.curGoods.gid != null
    ) {
  ...
  methods: {
    ...
    colsebuyGoodsPopupMethod(data) {
      if (data != undefined) this.cartAllTotalNums = data;
...
.back {
  /* 固定定位，浮在swiper上层，不占空间 */
  position: fixed;
  top: 10px; /* 顶部留10px间距，避免贴边（可调整） */
  left: 10px; /* 左侧留10px间距（可调整为right:10px靠右） */
  z-index: 999; /* 层级高于swiper，确保在最上层 */

  /* 透明背景核心 */
  background: transparent; /* 完全透明背景 */

  /* 点击区域优化 */
  padding: 8px 12px; /* 扩大点击区域，保证易点击 */
  border-radius: 4px; /* 可选：圆角，提升美观 */

  /* 可选：文字/图标可读性增强（防止和swiper内容混在一起） */
  backdrop-filter: blur(2px); /* 可选：轻微毛玻璃效果，提升文字清晰度 */
}

/* 按钮文字样式：白色+轻微阴影，适配不同背景的swiper */
.back-text {
  margin-left: 5px;
  font-size: 14px;
  color: #fff; /* 白色文字，适配轮播图的深色/浅色背景 */
  text-shadow: 0 0 2px #000; /* 轻微文字阴影，增强可读性 */
}

/* 轮播图样式：全屏展示，无需加margin-top */
.goodsswiper {
  width: 100%;
  height: 300px; /* 轮播图高度，根据需求调整 */
}
